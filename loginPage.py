...
# from voice_controlled_chatbot import chat
...
def Authenticate(*args):
    ...
    if emailVal=="Place your registered email...":
        ...
        if len(list1)!=0:   #valid email->email will be there...
...
def voice():
    global interfaceChooseVoice
    interfaceChooseVoice.destroy()
    os.system('python KhecVoiceBot/NEWAIASSISTANTBYBACH/integrate.py') 

def model_based():
    global interfaceChooseVoice
    interfaceChooseVoice.destroy()
    os.system('python khec_model/college_dataset/model.py')         

def text_vs_speech():
    global tvs
    tvs = Tk()
    tvs.geometry("300x250")
    tvs.title("Your Choice Our Responsibilities")
    Label(text="What is Your Choice:", bg="blue", width="300", height="2", font=("Calibri", 13)).pack()
    Label(text="").pack()
    Button(text="Text", height="2", width="30", command = text).pack()
    Label(text="").pack()
    Button(text="Speech", height="2", width="30", command=chooseVoice).pack()
 
def chooseVoice():
    global tvs
    tvs.destroy()
    global interfaceChooseVoice
    interfaceChooseVoice = Tk()
    interfaceChooseVoice.geometry("300x250")
    interfaceChooseVoice.title("Let's be more interactive")
    Label(text="Choose Your Interest:", bg="blue", width="300", height="2", font=("Calibri", 13)).pack()
    Label(text="").pack()
    Button(text="Real Time AI Assist", height="2", width="30", command = voice).pack()
    Label(text="").pack()
    Button(text="Model Based", height="2", width="30", command=model_based).pack() 
...
def sendOTP():
    ...
    mycursor = mydb.cursor()
    mycursor.execute(f"select * from `new_user_regis` where `email`='{email_name.get() }' ")
    lst_cursor=list(mycursor)
    if len(lst_cursor)!=0:  #email already exists
        messagebox.showinfo("Show info: Email already Exist","This account is already registered in database!!!")
        # CreateWidget()
        emailEntry.delete(0, 'end')
    else:
        numbers = "0123456789"
        ...
        login = "b242cb598fb54c" # paste your login generated by Mailtrap
        password = "96245963472b93" # paste your password generated by Mailtrap
        sender_email = "omitbajracharya55@gmail.com"
        # receiver_email = "omitbajracharya@gmail.com"
        receiver_email = email_name.get()
        Email=receiver_email
        
        message = MIMEMultipart(otpMSG)
        message["Subject"] = "Verification of email through otp"
        message["From"] = sender_email
        message["To"] = receiver_email
        # write the plain text part
        text = """\
        Hi,
        Check out the new post on the Mailtrap blog for verification through otp:
    ...
    #     smtp.login("omitbajracharya55@gmail.com", "khec_chat")
    #     # Sending the email with Mulitpart message converted into string
    #     smtp.sendmail("omitbajracharya@gmail.com", receiverEmail, message.as_string())
    #     # Terminating the session
    #     smtp.quit()
    
        # Formatting receiveEmail to replace(hide) mail id with *
        receiverEmail = '{}********{}'.format(receiver_email[0:2], receiver_email[-10:])
        # Displaying the success message
